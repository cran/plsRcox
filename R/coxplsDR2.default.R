coxplsDR2.default <- function(Xplan,time,time2,event,type,origin,typeres="deviance", collapse, weighted, scaleX=TRUE, scaleY=TRUE, ncomp=min(7,ncol(Xplan)), validation = "CV", plot=FALSE, allres=FALSE,...) {
library(pls)
...
mf3$data <- Xplan
mf3$method <- "oscorespls"
mf3[[1L]] <- as.name("plsr")
plsDR2_mod <- eval(mf3, parent.frame())
tt_plsDR2 <- data.frame(pls::scores(plsDR2_mod)[,])
