coxplsDR.default <- function(Xplan,time,time2,event,type,origin,typeres="deviance", collapse, weighted, scaleX=TRUE, scaleY=TRUE, nt=min(7,ncol(Xplan)),typeVC="none", plot=FALSE, allres=FALSE,sparse=FALSE,sparseStop=TRUE,...) {
...
plsDR_mod <- eval(mf3, parent.frame())
tt_plsDR <- data.frame(plsDR_mod$tt)
...
else {return(list(tt_plsDR=tt_plsDR, cox_plsDR=cox_plsDR, plsDR_mod=plsDR_mod))}
}
